...
export default {
    ...
    [TYPE.clientFunctionInterruptedByDialogError]: err => markup(err, `
        ${err.instantiationCallsiteName} execution was interrupted by a native ${err.dialogType} dialog with text "${err.dialogText}". Make sure that ${err.instantiationCallsiteName} code does not trigger native dialogs. If this dialog appears as a result of the previous test action, call a "handle..." function that handles this type of dialog after the action.
...
`),
    ...
    [TYPE.unexpectedDialogError]: err => markup(err, `
        An unexpected native ${err.dialogType} dialog with text "${err.dialogText}" appeared. If this dialog was invoked as a result of a test action, call a "handle..." function that handles this type of dialog after the action.

        ${err.getCallsiteMarkup()}
    `),

    [TYPE.expectedDialogNotAppearedError]: err => markup(err, `
        The expected native ${err.dialogType} dialog did not appear. Make sure that this dialog is invoked and the "handle..." function's "timeout" setting provides sufficient time for it to appear.
