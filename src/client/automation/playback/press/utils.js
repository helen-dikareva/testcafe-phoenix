...
export function excludeShiftModifiedKeys (keyArray) {
    var keysForKeyEvent = keyArray.slice();
    ...
    for (var i = 0; i < keyArray.length; i++) {
        ...
        if (key.toLowerCase() === 'shift') {
            ...
            if (KEY_MAPS.shiftMap[nextKey])
                ...
                keysForKeyEvent[i + 1] = KEY_MAPS.reversedShiftMap[nextKey];
        }
        ...
        if (KEY_MAPS.shiftMap[key] && (!keyArray[i - 1] || keyArray[i - 1].toLowerCase() !== 'shift')) {
            ...
            keysForKeyEvent.splice(i, 0, 'shift');
            i++;
        }
    }

    return { keys: keyArray, keysForKeyEvent };
}
