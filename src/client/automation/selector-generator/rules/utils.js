...
export function getIdSelector (el, idAttr) {
    if (ASP_AUTOGENERATED_ID_PART_RE.test(idAttr)) {
        ...
        return getRegExpAttributeSelector(RULE_TYPE.byId, el, tagName, filterValue);
    }

    return new SelectorObject({
        ruleType: RULE_TYPE.byId,
        element:  el,
        selector: '#' + escapeSpecifiedSymbols(idAttr)
    });
}
...
function getRegExpAttributeSelector (ruleType, el, selector, filterRegExpValue, ancestorSelector) {
    return new SelectorObject({
        ruleType:         ruleType,
        element:          el,
...
export function getAttributesSelector (ruleType, el, attributes, ancestorSelector) {
    var tagName         = domUtils.getTagName(el);
    var selector        = '';
    var attrRegExpValue = '';
    ...
    if (attrRegExpValue)
        return getRegExpAttributeSelector(ruleType, el, selector || tagName, attrRegExpValue, ancestorSelector);

    return new SelectorObject({ ruleType, element: el, ancestorSelector, selector });
}
...
export function isSimpleElementRule (ruleType) {
    return ruleType === RULE_TYPE.byTag ||
           ruleType === RULE_TYPE.byNameAttr ||
           ruleType === RULE_TYPE.byId ||
           ruleType === RULE_TYPE.byText ||
           ruleType === RULE_TYPE.byAttr;
}
